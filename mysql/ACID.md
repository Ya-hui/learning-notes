...
ps： 幻读和不可重复读很容易混淆，大致的区别在于幻读是由另一个事务插入或删除引起的，而不可重复是由于另一个事物的更改造成的，解决幻读需要锁表，解决不可重复读只需要锁住满足条件的行
...
1. 未提交读
    ...
    从上图可看出事务B还未提交事务A已经读到了事务B修改过的数据，一旦客户端B的事务回滚，那么客户端A查询到的数据其实就是脏数据，要想解决这个问题可以采用提交读的隔离级别
2. 提交读
